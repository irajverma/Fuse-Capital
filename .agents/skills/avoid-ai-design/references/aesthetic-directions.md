...
## 13. Finance / High-Trust Quantitative (Capital & Prop Trading)
**Fits:** fintech platforms, prop trading, capital management, financial analytics, investment apps.
**Type:** high-precision grotesque (Söhne, Neue Haas Grotesk, IBM Plex Sans) paired with mono for data (JetBrains Mono, IBM Plex Mono). All numerical values MUST use `tabular-nums` / `font-mono`.
**Palette:** solid obsidian dark (`#07090E`, `#0F172A`) or crisp paper light. Solid signal accents (emerald `#10B981`, crimson `#EF4444`, amber `#F59E0B`, or gold/platinum for capital tiers). **STRICT ZERO DECORATIVE GRADIENTS.** Solid, flat colors only.
**Layout:** dense, crisp 1px borders (`border-white/10` or `border-slate-800`), structured data grids, sharp financial metric blocks. **STRICT ZERO PILL BADGES / TAGS.**
**Motion:** instant, precise transitions (100-150ms max). No playful bounces or floating animations.
**Signature:** tabular numbers, crisp 1px gridlines, functional status indicators (minimal green/red status dots), institutional authority and high-trust clarity.

---

## Quick chooser

- **Finance / Capital / Prop Trading**: Finance / High-Trust Quantitative, Luxury / Refined, Swiss / International Typographic
- **Dev tool / honest**: Brutalist, Industrial, Monospace
- **Content / voice**: Editorial, Maximalist
- **Data / clarity**: Swiss, Industrial, Finance / High-Trust Quantitative
- **Premium**: Luxury, Art deco, Finance / High-Trust Quantitative
- **Consumer / joyful**: Playful, Organic
- **Calm SaaS**: Warm minimal (only if executed precisely), Organic

When two fit, blend deliberately (e.g. Finance data density on a Swiss grid). When none obviously fits, default to the one the *audience* respects, then commit hard.
