...
### C1: The purple/indigo-to-blue diagonal or random gradient · P0
**Detection:** `linear-gradient(...)` from indigo/violet to blue, or multi-hue decorative background gradients in the hero, CTA, cards, or background glow.
**Why AI:** The canonical tell, "the Purple Problem." Multi-hue gradients are decorative filler reached for without intent. In financial UIs, random gradients destroy credibility and read as cheap templates.
**Fix (HTML/CSS):** Use flat, solid brand colors or crisp 1px borders. If dark mode is used, use a deep solid obsidian/slate background (`#0B0F17` or `#05070B`). Never use decorative multi-hue gradients.
```css
:root { --background:#07090e; --surface:#0f172a; --border:rgba(255,255,255,0.1); --accent:#10b981; } /* Solid, crisp financial palette */
```
**Fix (React/Tailwind):** Replace `bg-gradient-to-br from-indigo-500 to-purple-600` with solid theme tokens (`bg-slate-950`, `bg-zinc-900`). A flat, high-contrast surface with a sharp 1px border beats any gradient.
...
**Fix:** Tie the primary action to the financial brand's dominant or accent color (e.g. emerald `#10B981`, gold/amber `#F59E0B`, or sharp solid white `#FFFFFF` on dark). Give it a real hover and active state (see K7).
...
**Fix:** Pick a dark palette with intent: a rich obsidian near-black, high-contrast text, and financial signal colors (emerald green, ruby red, amber). Avoid grey-on-grey body text.
...
### C5: Colored glow box-shadows & ambient gradient blobs · P0
**Detection:** Cards or buttons with vibrant colored shadows (`shadow-indigo-500/50`) or blurred gradient circles floating in the background.
**Why AI:** Pure decorative fluff that ruins high-trust financial interfaces.
**Fix:** Remove all gradient glow blobs and colored shadows. Use crisp 1px borders (`border-slate-800` or `border-white/10`) for card boundaries, and subtle neutral elevation shadows only when floating layers exist.

### C6: Gradient headline text · P0
**Detection:** `bg-clip-text text-transparent bg-gradient-to-r ...` on a heading.
**Why AI:** A default flourish that ruins legibility and screams AI generator.
**Fix:** Make headings solid high-contrast text (`text-slate-900` or `text-white`). For emphasis use font weight, size, or a solid accent color, never a gradient fill.
...
**Fix (HTML/CSS):** Break symmetry. Try a left-aligned hero with an asymmetric financial dashboard preview, a split layout, or a strict grid. Let one element be dramatically larger.
**Fix (React/Tailwind):** Replace the `flex flex-col items-center text-center` hero with a `grid` that places headline, supporting metrics, and media on an intentional grid. Drop the pill badge entirely.
...
**Fix:** Use radius and elevation to express hierarchy, not as a global default. In financial UIs, prefer sharp or small radiuses (`rounded-sm`, `rounded-md`, or `rounded-none`) with crisp 1px borders to reflect precision.
...
**Fix:** Use a translucent surface only where layering is real (a sticky header over scrolling content). Everywhere else, use a solid surface with a considered color.
...
### K5: Pill badges, floating tags, and sparkle chips · P0
**Detection:** Floating capsule pills, category badges, tag badges, or sparkle chips above headings (e.g. "✨ New: v2 is here", "FEATURE", "PRO").
**Why AI:** The ultimate generic AI ornament. Pill badges clutter the visual hierarchy and look like cheap SaaS templates.
**Fix:** Ban all pill badges and floating tags. Replace them with clean typographic section headers, an uppercase eyebrow kicker in solid muted text, or a structured data row. For metric status, use a minimal status dot (`w-2 h-2 rounded-full bg-emerald-500`) paired with crisp solid text.
...
**Fix:** Set a radius that fits the direction: sharp (0 to 4px) for financial precision and editorial clarity.
...
**Fix:** Choose a real dark palette (a rich obsidian or deep navy) with crisp 1px borders and distinct surface layers.

---

## Finance UI Specifics

### FIN1: Non-tabular metrics and financial numbers · P1
**Detection:** Financial metrics, balances, P&L numbers, percentages, or trading figures using proportional fonts without tabular spacing.
**Why AI:** Proportional font numbers wobble and shift when numbers update, destroying financial rigor and precision.
**Fix:** Apply `font-variant-numeric: tabular-nums` or use a clean monospace font (`font-mono`) for all financial values, prices, percentages, and metrics.

### FIN2: Soft blurry card shadows instead of crisp borders · P1
**Detection:** Financial tables, position cards, or analytics widgets relying on soft blurry shadows rather than crisp 1px border gridlines.
**Why AI:** Blur shadows obscure boundary lines and ruin data grid clarity.
**Fix:** Use sharp 1px borders (`border-slate-800` or `border-white/10`) to delimit data containers and table cells cleanly.

### FIN3: Toy badges for financial metrics / account tiers · P0
**Detection:** Pill-shaped rounded tags or pastel badges used for account balances, profit targets, or risk limits.
**Why AI:** Looks like a casual toy app rather than an institutional capital platform.
**Fix:** Present financial status and risk tiers with structured key-value pairs, clean typography, or a minimal solid status indicator dot.
