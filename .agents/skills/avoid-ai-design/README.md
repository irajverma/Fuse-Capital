...
|---|---|
| **Typography** | Inter / Roboto / system default, no display face, non-tabular financial numbers, overused "safe" picks |
| **Color** | purple-to-blue or decorative gradients, gradient `bg-clip-text` headline text, ambient glow blobs, untouched shadcn `zinc`/`slate`, timid palettes, default `blue-600` buttons |
| **Layout** | centered hero, the hero + three-feature-cards + CTA template, default page shell, three-tier pricing rings, four-column footer, zero asymmetry |
| **Components** | floating pill badges / sparkle chips, `rounded-2xl shadow-lg` on everything, glassmorphism by reflex, icon-in-a-rounded-square, default unstyled Card/Button |
| **Finance UI** | non-tabular metrics, soft blurry card shadows instead of sharp 1px borders, toy badges on financial account data |
...
3. **Direction.** Commit to one concrete aesthetic direction for the artifact (for example Finance / High-Trust Quantitative, brutalist editorial, warm editorial pastel, or refined luxury), named with three to five defining moves. You confirm it before any code changes.
...
- **P0** screams AI on sight: decorative pill badges / sparkle chips, purple-to-blue or random gradients, `bg-clip-text` gradient headlines, ambient gradient glow blobs, Inter everywhere, centered hero-plus-three-cards template, untouched shadcn zinc, reflexive glassmorphism.
- **P1** is the obvious AI smell: `rounded-2xl shadow-lg` on every surface, non-tabular financial figures, icon-in-rounded-square, emoji feature bullets, default blue buttons, "Elevate your…" copy.
