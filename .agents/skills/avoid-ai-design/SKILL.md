...
- **P0, a layperson recognizes it as AI-made.** The purple-to-blue gradient or any random multi-hue gradient, decorative pill badges / pill tags above headers, Inter for everything, an untouched shadcn base theme, gradient (`bg-clip-text`) headline text, reflexive glassmorphism. These are the memes.
- **P1, a designer or developer recognizes it.** `rounded-2xl shadow-lg` on every surface, the default page shell (`container mx-auto px-4`, `max-w-7xl`), icon-in-a-rounded-square, non-tabular financial numbers, the default four-column footer, dead hover/focus states, arrow glyphs stapled to CTAs, default-blue or indigo buttons, "Elevate your workflow" copy.
...
|---|---|
| `finance-platform` / `finance-marketing` | Strict precision. Zero pill badges, zero random gradients. Flat solid surfaces, crisp 1px borders, high data density, tabular figures (`tabular-nums`), and high-trust financial color palettes (obsidian, slate, emerald, gold/amber). |
...
- **No Pill Badges / Tags.** Floating pill badges, pill tags, and sparkle chips are strictly banned. Replace them with clean typographic headers or subtle section kickers.
- **No Decorative / Random Gradients.** Multi-hue background gradients, `bg-clip-text` gradient headlines, and ambient glow blobs are strictly banned. Use flat, high-contrast solid colors and clean surfaces.
- **Finance Precision & Trust.** Financial UIs require maximum legibility, tabular number alignment (`font-variant-numeric: tabular-nums` or `font-mono`), crisp 1px border dividers (`border-white/10` or `border-slate-800`), and functional status indicators (solid green/red/amber status dots) rather than decorative flares.
